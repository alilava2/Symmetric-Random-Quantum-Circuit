...
p_unitaries=[0.0]
p_singles=collect(0.4:0.01:0.6)
...
@everywhere include("GKToolbox.jl");
@everywhere using .GKToolbox
...
for i in eachindex(workers())
    remote_do(mineTheDataU, workers()[i], ns,p_unitaries,p_singles, ensembleSize[i], resultsChannel)
end
...
@time for _ in 1:validPairsNum*length(ns)*nworkers()
    ...
    if !isassigned(gatherdData,n_id,p_unitary_id,p_single_id)
        ...
        gd.initialEntProf+=rd.initialEntProf
